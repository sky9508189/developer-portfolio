(function () {
  ...
  const state = {
    ...
    experience: [
      { company: 'TechCorp', role: 'Senior Developer', period: '2022G��Present', desc: 'Led frontend architecture for SaaS platform serving 50k+ users' },
      { company: 'StartupX', role: 'Full-Stack Developer', period: '2020G��2022', desc: 'Built and shipped MVP within 3 months using React and Node.js' },
    ],
    education: [
      { institution: 'MIT', degree: 'B.S. Computer Science', period: '2016G��2020' },
    ],
  };
  ...
  function renderProjectCard(item, i) {
    return `<div class="item-card">
      <div class="item-header"><span class="item-title">Project ${i+1}</span><div class="item-actions"><button class="item-del" data-idx="${i}">G��</button></div></div>
  ...
  function renderExpCard(item, i) {
    return `<div class="item-card">
      <div class="item-header"><span class="item-title">Experience ${i+1}</span><div class="item-actions"><button class="item-del" data-idx="${i}">G��</button></div></div>
      <div class="item-fields">
  ...
  function renderEduCard(item, i) {
    return `<div class="item-card">
      <div class="item-header"><span class="item-title">Education ${i+1}</span><div class="item-actions"><button class="item-del" data-idx="${i}">G��</button></div></div>
      <div class="item-fields">
...
<title>${esc(name)} G�� Portfolio</title>
...
<div class="container">
  ...
  ${exp.length ? `<section><h2>Experience</h2>${exp.filter(e => e.company).map(e => `
    <div class="exp-item">
      <div class="exp-header"><strong>${esc(e.company)}</strong> ${e.role ? `G�� ${esc(e.role)}` : ''}</div>
  ...
  ${edu.length ? `<section><h2>Education</h2>${edu.filter(e => e.institution).map(e => `
    <div class="exp-item">
      <div class="exp-header"><strong>${esc(e.institution)}</strong> ${e.degree ? `G�� ${esc(e.degree)}` : ''}</div>
      ${e.period ? `<div class="exp-period">${esc(e.period)}</div>` : ''}
    </div>
  `).join('')}</section>` : ''}
...
.contact-row a{color:rgba(255,255,255,.6);border-bottom:1px solid rgba(255,255,255,.1)}
.contact-row a:hover{color:#fff;border-color:rgba(255,255,255,.3)}
.bio-text{color:rgba(255,255,255,.55)}
.skill-tag{background:rgba(255,255,255,.04);color:rgba(255,255,255,.7);border:1px solid rgba(255,255,255,.08);backdrop-filter:blur(7px)}
.project-card{background:rgba(255,255,255,.03);backdrop-filter:blur(7px);-webkit-backdrop-filter:blur(7px);border:1px solid rgba(255,255,255,.06);box-shadow:0 4px 20px rgba(0,0,0,.2)}
.project-card:hover{background:rgba(255,255,255,.06);border-color:rgba(255,255,255,.12)}
.project-card h3{color:rgba(255,255,255,.85)}
.project-links a{color:rgba(100,200,255,.7);border-bottom:1px solid rgba(100,200,255,.15)}
